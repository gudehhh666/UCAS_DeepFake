...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        all_preds.extend(F.softmax(output, dim=1)[:, 1].detach().cpu().numpy())
        all_outputs.extend(output.detach().cpu().numpy())

        metric_logger.update(loss=loss.item())


    assert len(all_outputs) == len(all_targets) == len(all_preds)
    print(f'all_outputs: {len(all_outputs)}')

    acc_total = float(accuracy(torch.tensor(all_outputs,dtype=torch.float32), torch.tensor(all_targets,dtype=torch.float32), topk=(1,))[0])
    try:
        auc_score = roc_auc_score(torch.tensor(all_targets,dtype=torch.float32), torch.tensor(all_preds,dtype=torch.float32)) * 100.
    ...
    # metric_logger.meters['acc'].update(acc_total, n=batch_size)
    ...
    # print('* Auc {auc.global_avg:.3f}  loss {losses.global_avg:.3f} acc {acc.global_avg:.3f}'
    #       .format(auc=metric_logger.auc, losses=metric_logger.loss, acc=metric_logger.acc))   
    print(f'* Auc {auc_score:.3f} acc {acc_total:.3f}')

    # return {k: meter.global_avg for k, meter in metric_logger.meters.items()}
    return auc_score, acc_total
...
def test_binary_video_frames(data_loader, model, device):
    ...
    print(f'result preview: \nBalanced_Acc {metric_logger.frame_balanced_acc.global_avg:.3f} \nAuc {metric_logger.frame_auc.global_avg:.3f} ', file=sys.stderr)
