...
def save_model_target_network(args, epoch, model, model_target_network_without_ddp, optimizer, loss_scaler, tag=None):
    output_dir = Path(args.output_dir)
    epoch_name = str(epoch)
    if loss_scaler is not None:
        if tag is None:
        ...
        for checkpoint_path in checkpoint_paths:
            to_save = {
                'model': model_target_network_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
                'epoch': epoch,
                'scaler': loss_scaler.state_dict(),
                'args': args,
...
def load_model_target_network(args, model_target_network_without_ddp, optimizer, loss_scaler):
    if args.resume:
        if args.resume.startswith('https'):
            checkpoint = torch.hub.load_state_dict_from_url(
                args.resume, map_location='cpu', check_hash=True)
        else:
            checkpoint = torch.load(args.resume_target_network, map_location='cpu')
        model_target_network_without_ddp.load_state_dict(checkpoint['model'])
        print("Resume checkpoint %s" % args.resume_target_network)
        if 'optimizer' in checkpoint and 'epoch' in checkpoint and not (hasattr(args, 'eval') and args.eval):
            optimizer.load_state_dict(checkpoint['optimizer'])
            args.start_epoch = checkpoint['epoch'] + 1
            if 'scaler' in checkpoint:
